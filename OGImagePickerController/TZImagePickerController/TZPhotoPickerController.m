...
//    _shouldScrollToBottom = YES;
    ...
    if (self.allowCamera) {
        [_photoArr addObject:[TZAssetModel modelWithAsset:[UIImage imageNamed:@"og_camera"]
                                                     type:TZAssetModelMediaTypeCamera]];
    }
    ...
    [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:imagePickerVc.allowPickingVideo completion:^(NSArray<TZAssetModel *> *models) {
        ...
        [_photoArr addObjectsFromArray:reversedArray];
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[OGImageCollectionViewCell class] forCellWithReuseIdentifier:@"OGImageCollectionViewCell"];
}
...
- (void)okButtonClick {
    ...
    for (NSInteger i = 0; i < _selectedPhotoArr.count; i++) {
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            if (model) [assets replaceObjectAtIndex:i withObject:model];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeCamera) {
        OGImageCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"OGImageCollectionViewCell" forIndexPath:indexPath];
        cell.imageView.contentMode = UIViewContentModeCenter;
        cell.imageView.image = model.asset;
        return cell;
    }else{
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeCamera) {
        ...
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            //			拍照
            [self getPicFrom:UIImagePickerControllerSourceTypeCamera andTag:0];
        }else{
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"设备不支持该拍照" message:nil delegate:nil cancelButtonTitle:@"知道了" otherButtonTitles:nil, nil];
            [alertView show];
        }
...
- (void)getPicFrom:(UIImagePickerControllerSourceType)sourceType andTag:(NSInteger)tag
{
...
//        UIImage *fixImage = [image fixOrientation];
        TZAssetModel *a_model = [TZAssetModel modelWithAsset:image type:TZAssetModelMediaTypePhoto];
        a_model.isSelected = YES;
        _selectedPhotoArr = [NSMutableArray arrayWithArray:@[a_model]];
        [self okButtonClick];
