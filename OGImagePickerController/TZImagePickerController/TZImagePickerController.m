...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate assets:(NSArray *)assets currentIndex:(NSUInteger)currentIndex{
    
    TZPhotoPreviewController *photoPreviewVc = [[TZPhotoPreviewController alloc] init];
    photoPreviewVc.photoArr = assets;
    photoPreviewVc.isSelectOriginalPhoto =_allowPickingOriginalPhoto;
    photoPreviewVc.currentIndex = currentIndex;
    ...
    photoPreviewVc.okButtonClickBlock = ^(NSMutableArray *newSelectedPhotoArr,BOOL isSelectOriginalPhoto){
        ...
        if (newSelectedPhotoArr.count == 0) {
            ...
            [imagePickerVc showProgressHUD];
            for (NSInteger i = 0; i < newSelectedPhotoArr.count; i++) { [photos addObject:@1];[assets addObject:@1];[infoArr addObject:@1]; }
            
            for (NSInteger i = 0; i < newSelectedPhotoArr.count; i++) {
                TZAssetModel *model = newSelectedPhotoArr[i];
    ...
    self = [super initWithRootViewController:photoPreviewVc];
    if (self) {
        self.maxImagesCount = maxImagesCount > 0 ? maxImagesCount : 9; // Default is 9 / 默认最大可选9张图片
        self.pickerDelegate = delegate;
        // Allow user picking original photo and video, you also can set No after this method
